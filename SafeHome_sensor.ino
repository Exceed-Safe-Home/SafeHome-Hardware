...
//Flame, Water, Shock
int LED_input[3][2] = {{15, 33}, {0, 2}, {14, 32}}; //Red, Green pin  
...
void loop(){
  ...
  //flame 2700-500-300, gas 2000up-2000down, smoke 1800-2000 || LED1
  for(int i=0;i<3;i++) digitalWrite(buzzer[i], LOW); //reset

  if(Gas>950){ //gas off limit
  ...
  if(Smoke>1800) stateSmoke=2;
  else if(Smoke>1500) stateSmoke=1;
  else stateSmoke=0;
  
  if(!stateFlame&&!stateSmoke){ //0&0
    for(int i=0;i<2;i++) ledcWrite(i, LED_Mode[0][i]); //green
    digitalWrite(led_in, HIGH); //spinker off
  } else if(stateFlame+stateSmoke < 3){ //middle
    for(int i=0;i<2;i++) ledcWrite(i, LED_Mode[1][i]); //yellow
    digitalWrite(led_in, HIGH); //spinker off
  } else {
    for(int i=0;i<2;i++) ledcWrite(i, LED_Mode[2][i]); //red
    digitalWrite(buzzer[0], HIGH); //Warning
